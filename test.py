...
class App:
    def __init__(self, root):
        ...
        header_frame = tk.Frame(root,border=5,relief=RAISED,bg="red")
        header_frame.pack(side=TOP,pady=30)
        GLabel_907=tk.Label(header_frame)
        GLabel_907["bg"] = "#00ff77"
        GLabel_907["disabledforeground"] = "#000000"
        ft = tkFont.Font(family='Consolas',size=18)
        GLabel_907["font"] = ft
        GLabel_907["fg"] = "#4d00ff"
        ...
        content_frame = tk.Frame(root,border=2,bg="white")
        content_frame.pack(side=TOP,padx=20)
        
        #This frame contains only the label widgets
        label_frame = tk.Frame(content_frame,bg="red")
        ...
        mid_frame = tk.Frame(root,border=2,bg='white')
        mid_frame.pack(side=TOP,pady=50)
        
        #This frame contains the last matching results
        self.last_frame = tk.Frame(root,border=2,bg='white')
    ...
    def findGames(self):
            ...
            for game in self.data:
                if game['min_players'] <= number_of_players <= game['max_players'] and game['duration']<=time_available and game['min_age']<=youngest_age :
                    if game['name'] not in self.included:
                        ...
                        for i in range(len(self.games_available)):
                            if self.games_available[i]['name'] == games['name']:
                                self.games_available.pop(i)
        ...
        ### Matching Games Heading ###
            ...
            if len(found)<1:
                ...
                if len(found)>1:
                    tk.messagebox.showinfo('Total Matches',f'{str(len(found))} out of {str(len(self.data))} games matched the search')
                else:
                    tk.messagebox.showinfo('Total Matches',f'{str(len(found))} out of {str(len(self.data))} game matched the search')
